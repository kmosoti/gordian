#!/usr/bin/env bash
# The target-crate issue-body contracts must agree with the native Atom snapshot.
set -euo pipefail
cd "$(dirname "$0")/.."
...
for required in "$module" "$order" "$snapshot"; do
  if [ ! -f "$required" ]; then
    echo "FAIL: $required is missing; the target-crate checker requires the registry module, execution order, and snapshot."
    exit 1
  fi
done

PYTHONPATH="orchestration/src${PYTHONPATH:+:$PYTHONPATH}" python3 -m gordian_orchestration.atom_registry \
  --snapshot "$snapshot" check-target-crates
